 ...
 */
YAHOO.widget.eZDebugNode = function(oData, oParent, expanded, name, editable) {
...
YAHOO.extend(YAHOO.widget.eZDebugNode, YAHOO.widget.Node, {
	...
	initContent: function(oData, expanded, name, editable) {
		...
		// build recursively all children nodes if this xmlrpcval object is array or struct
		if (oData.type.slice(0, 5) == 'array') {
		    if (oData.value !== null)
		    {
		        for (var i = 0; i < oData.value.length; i++) {
    				var newnode = new YAHOO.widget.eZDebugNode(oData.value[i], this, expanded, null, editable);
    			}
		    }
		...
		else if (oData.type.slice(0, 6) == 'object') {
			/// @todo implement data hiding (???)
			for (var attr in oData.value) {
				var newnode = new YAHOO.widget.eZDebugNode(oData.value[attr], this, expanded, attr, editable);
			}
...
/*		moveUpButtons[moveUpButtons.length] = 'ygtvu' + this.index;
	...
	getNodeHtml: function() {
		...
		*/
		...
		sb[sb.length] = '<td';
		if (this.depth == 0 && (this.data.type.slice(0, 6) == 'object' || this.data.type.slice(0, 5) == 'array') && !allowTopLevelElementTypeChange) {
			...
			//sb[sb.length] = getNode + '.getStyle()+\'_mvdown\'"';
			if (this.depth > 0 || this.data.type == 'undef') {
		...
		sb[sb.length] = '<td';
		if (/*this.depth > 0 ||*/ this.data.type.slice(0, 6) == 'object' || this.data.type.slice(0, 5) == 'array') {
		...
		if (this.data.type.slice(0, 5) != 'array' && this.data.type.slice(0, 6) != 'object') {
			sb[sb.length] = '<span';
...
/*		if (this.data.type.slice(0, 5) == 'array') {
			sb[sb.length] = ' >['+this.data.scalarTyp()+' ('+this.data.arraySize()+')]</span>';
		}
		else if (this.data.type.slice(0, 6) == 'object') {
...
/*	onDelClick: function(me) {
		...
		if (this.data.type == 'array' || this.data.type == 'struct') {
...
/*function editElementOk() {
	...
	if (preventAnonStructEelements) {
		if (newnam == '' && editingElement.depth > 0 && editingElement.parent.data.type == 'struct') {
...
/*function addElementOk() {
	...
	if (preventAnonStructEelements) {
		if (newnam == '' && editingElement.data.type == 'struct') {
			alert('Sorry, a name is needed for struct elements');
			return;
		}
	}
	...
	if (editingElement.data.type == 'struct') {
	...
	else if (editingElement.data.type == 'array') {
...
/**
 * Build a new xmlrpcval object, doing a little type validation
 * @access public
 * @return xmlrpcval
 **/
